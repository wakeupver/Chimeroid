            ...
            ChimeroidControlFaceButtons(
                ...
                idsForegrounds =
                    persistentMapOf<Id.Key, @Composable (State<Boolean>) -> Unit>(
                        Id.Key(KeyEvent.KEYCODE_BUTTON_X) to { ChimeroidCentralButton(pressedState = it, label = "Y") },
                        Id.Key(KeyEvent.KEYCODE_BUTTON_L1) to { ChimeroidCentralButton(pressedState = it, label = "X") },
                        Id.Key(KeyEvent.KEYCODE_BUTTON_B) to { ChimeroidCentralButton(pressedState = it, label = "B") },
                        Id.Key(KeyEvent.KEYCODE_BUTTON_Y) to { ChimeroidCentralButton(pressedState = it, label = "A") },
                    ),
            )
        },
        secondaryDials = {
