...
class BiosManager(private val directoriesManager: DirectoriesManager) {
    ...
    fun getMissingBiosFiles(
        ...
        // All files in requiredBIOSFiles must be present.
        val missingRequired = (coreConfig.requiredBIOSFiles + requiredRegionalFiles)
            .filter { !File(directoriesManager.getSystemDirectory(), it).exists() }

        // For requiredBIOSFilesAnyOf: at least ONE file from the list must exist.
        // If none exist we report ALL of them as missing so the UI can tell the user
        // which files are accepted.
        val missingAnyOf = if (coreConfig.requiredBIOSFilesAnyOf.isNotEmpty()) {
            val anyPresent = coreConfig.requiredBIOSFilesAnyOf
                .any { File(directoriesManager.getSystemDirectory(), it).exists() }
            if (anyPresent) emptyList() else coreConfig.requiredBIOSFilesAnyOf
        } else {
            emptyList()
        }

        return missingRequired + missingAnyOf
    }
