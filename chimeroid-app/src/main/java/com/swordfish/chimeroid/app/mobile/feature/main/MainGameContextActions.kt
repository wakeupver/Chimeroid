...
import com.swordfish.chimeroid.app.mobile.shared.compose.ui.ChimeroidGameTexts
import com.swordfish.chimeroid.app.mobile.shared.compose.ui.ChimeroidSmallGameImage
...
fun MainGameContextActions(
    ...
    // WindowBottomSheet keeps `content` mounted through the dismiss animation
    // (only `show` toggles), so — unlike the plain-null selectedGame — we keep
    // rendering the last non-null game while that animation plays out instead
    // of blanking the sheet the instant selection is cleared.
    var displayedGame by remember { mutableStateOf(selectedGame) }
    if (selectedGame != null) {
        displayedGame = selectedGame
    }
    val gameToShow = displayedGame
