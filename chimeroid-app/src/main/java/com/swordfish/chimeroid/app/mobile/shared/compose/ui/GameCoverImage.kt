 ...
 * shared fallback/error drawable lookup. Every cover-art surface in the app (grid cards,
 * list rows, small thumbnails, bento tiles) renders through this composable so the
 * request-building and fallback logic exists exactly once.
 *
 * Both the [ImageRequest] and the fallback [rememberDrawablePainter] are keyed on [game],
 * so recomposition caused by unrelated state changes reuses the previous instances instead
 * of re-allocating them every frame.
 */
@Composable
fun GameCoverImage(
