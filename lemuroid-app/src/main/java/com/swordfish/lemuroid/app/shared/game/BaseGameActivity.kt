...
abstract class BaseGameActivity : ImmersiveActivity() {
     ...
     *   1. Call retro_cheat_reset() first to clear ALL existing cheat slots.
     *   2. Only pass cheats that the user has toggled ON.
     *   3. Re-index them starting from 0 so slot numbers are contiguous.
     *
     * Without the reset step, cheats that were previously enabled but later disabled
     * would persist in their libretro slot and remain active.
     */
    private suspend fun applyPatchCodesToEmulator(retroView: com.swordfish.libretrodroid.GLRetroView) {
        val enabledCodes = patchCodesManager.getEnabledCodesForGame(game.id)
        retroView.resetCheats()
    ...
    private fun displayOptionsDialog(
        ...
        // Build the curated advanced options first.
        val curatedAdvancedOptions =
            systemCoreConfig.exposedAdvancedSettings
                .mapNotNull { transformExposedSetting(it, coreOptions) }

        // Auto-detect: collect every variable reported by the core that is NOT already
        // covered by exposedSettings or exposedAdvancedSettings. This mirrors RetroArch's
        // behaviour of showing all core options without requiring a hard-coded list.
        val autoDetectedOptions: List<LemuroidCoreOption> =
            if (systemCoreConfig.autoDetectSettings) {
                val curatedKeys =
                    (systemCoreConfig.exposedSettings + systemCoreConfig.exposedAdvancedSettings)
                        .map { it.key }
                        .toHashSet()
                coreOptions
                    .filter { it.variable.key !in curatedKeys }
                    .map { LemuroidCoreOption.fromCoreOption(it) }
            } else {
                emptyList()
            }

        // Auto-detected options are appended after the curated advanced ones so that
        // hand-crafted settings always appear first.
        val advancedOptions = curatedAdvancedOptions + autoDetectedOptions
