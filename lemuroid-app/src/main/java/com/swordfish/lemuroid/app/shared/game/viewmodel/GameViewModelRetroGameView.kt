...
import com.swordfish.lemuroid.lib.game.AdaptivePerformanceManager
import com.swordfish.lemuroid.lib.library.GameSystem
import com.swordfish.lemuroid.lib.library.SystemCoreConfig
import com.swordfish.lemuroid.lib.library.db.entity.Game
import com.swordfish.lemuroid.lib.storage.RomFiles
import com.swordfish.libretrodroid.GLRetroView
import com.swordfish.libretrodroid.GLRetroView.GLRetroEvents
...
class GameViewModelRetroGameView(
    ...
    override fun onCreate(owner: LifecycleOwner) {
        ...
        owner.launchOnState(Lifecycle.State.RESUMED) {
            initializeAdaptivePerformanceFlow()
        }
    }
     ...
     * [AdaptivePerformanceManager].  Every 3 s it reads the performance advice
     * and applies [GLRetroView.skipDuplicateFrames] accordingly.
     *
     * This is entirely non-blocking: the actual GL work runs on the GL thread;
     * we only observe its events here on the main/default dispatcher.
     */
    private suspend fun initializeAdaptivePerformanceFlow() {
        try {
            val retroView = retroGameViewFlow()

            // Feed frame events into the APM ring buffer
            scope.launch(Dispatchers.Default) {
                retroView.getGLRetroEvents()
                    .filterIsInstance<GLRetroEvents.FrameRendered>()
                    .collect { adaptivePerformanceManager.onFrameRendered() }
            }

            // Periodically apply advice (every 3 s is cheap enough).
            // IMPORTANT: if the SystemCoreConfig forces skipDuplicateFrames = false
            // (e.g. Flycast, SwanStation, PPSSPP, Mupen64Plus, DeSmuME) we must
            // never override that with the APM's opinion — doing so causes the
            // classic "double speed" bug where hardware-rendered cores run at 2×
            // speed because the APM enables frame-skipping before warmup completes.
            scope.launch(Dispatchers.Default) {
                while (true) {
                    delay(3.seconds)
                    ...
                    Timber.d(
                        ...
                        advice.thermalWarn,
                        advice.thermalCritical,
                    )
                    retroView.skipDuplicateFrames =
                        if (systemCoreConfig.skipDuplicateFrames) {
                            advice.skipDuplicateFrames
                        } else {
                            false
                        }
            ...
            Timber.w(e, "AdaptivePerformanceManager failed to start")
        }
    }
