...
class GLRetroView(
    ...
    var skipDuplicateFrames: Boolean by Delegates.observable(false) { _, _, value ->
        LibretroDroid.setSkipDuplicateFrames(value)
    }
    ...
    /** Returns the core's base video width (used for Square Pixel aspect ratio). */
    fun getVideoWidth(): Int = LibretroDroid.getVideoWidth()

    /** Returns the core's base video height (used for Square Pixel aspect ratio). */
    fun getVideoHeight(): Int = LibretroDroid.getVideoHeight()

    /**
     * Override the aspect ratio used by VideoLayout.
     * Pass ratio > 0 to force a specific AR; pass 0f (or negative) to revert to the
     * core's native aspect ratio (same effect as [refreshAspectRatio]).
     */
    fun setAspectRatioOverride(ratio: Float) {
        LibretroDroid.setAspectRatioOverride(ratio)
    }
    ...
    inner class Renderer : GLSurfaceView.Renderer {
        ...
        override fun onSurfaceChanged(gl: GL10, width: Int, height: Int) = catchExceptions {
            // THREAD_PRIORITY_URGENT_DISPLAY (-8) is higher than Thread.MAX_PRIORITY (10 Java).
            // This puts the GL/emulation thread in the same class as the compositor,
            // dramatically reducing scheduling jitter under CPU load.
