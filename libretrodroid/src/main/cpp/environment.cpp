...
bool Environment::handle_callback_environment(unsigned cmd, void *data) {
    switch (cmd) {
        ...
        case RETRO_ENVIRONMENT_GET_AUDIO_VIDEO_ENABLE:
            ...
            // Bit 0 = enable video, bit 1 = enable audio.  Always enable both.
            // Returning false here makes some cores (e.g. Mupen64Plus-Next) disable
            // their video or audio output paths entirely, producing black screens or silence.
            if (data != nullptr) {
        ...
        case RETRO_ENVIRONMENT_GET_INPUT_BITMASKS:
            // Returning true tells the core we support packed bitmask input polling.
            // Cores that query RETRO_DEVICE_ID_JOYPAD_MASK get all button states in
            // one call instead of 16 individual calls — reduces JNI overhead per frame.
            LOGD("Called RETRO_ENVIRONMENT_GET_INPUT_BITMASKS");
            return true;

        case RETRO_ENVIRONMENT_GET_CORE_OPTIONS_VERSION:
            // Advertise v2 support so cores skip the legacy SET_VARIABLES path.
        ...
        case RETRO_ENVIRONMENT_SET_CORE_OPTIONS: {
            // Cores that use this (v1) send an array of retro_core_option_definition.
            // Parse exactly like SET_VARIABLES: use first value as default if the key
            // hasn't been set by the app yet.
            LOGD("Called RETRO_ENVIRONMENT_SET_CORE_OPTIONS");
            if (data == nullptr) return true;
            const auto* defs = static_cast<const struct retro_core_option_definition*>(data);
            for (unsigned i = 0; defs[i].key != nullptr; ++i) {
                std::string key(defs[i].key);
                auto& var = variables[key];
                var.key = key;
                var.description = defs[i].desc ? defs[i].desc : key;
                if (var.value.empty() && defs[i].default_value && defs[i].default_value[0] != '\0') {
                    var.value = defs[i].default_value;
                } else if (var.value.empty() && defs[i].values[0].value != nullptr) {
                    var.value = defs[i].values[0].value;
                }
                LOGD("CoreOption v1 %s = %s", key.c_str(), var.value.c_str());
            }
            return true;
        }

        case RETRO_ENVIRONMENT_SET_CORE_OPTIONS_INTL: {
            // Use the US (English) option list from the intl struct.
            LOGD("Called RETRO_ENVIRONMENT_SET_CORE_OPTIONS_INTL");
            if (data == nullptr) return true;
            const auto* intl = static_cast<const struct retro_core_options_intl*>(data);
            ...
            for (unsigned i = 0; defs[i].key != nullptr; ++i) {
                ...
                LOGD("CoreOption v1_intl %s = %s", key.c_str(), var.value.c_str());
            }
            return true;
        }

        case RETRO_ENVIRONMENT_SET_CORE_OPTIONS_V2: {
            // v2 definitions include category grouping — we only care about key/default_value.
            LOGD("Called RETRO_ENVIRONMENT_SET_CORE_OPTIONS_V2");
            if (data == nullptr) return true;
            const auto* opts = static_cast<const struct retro_core_options_v2*>(data);
            if (opts->definitions == nullptr) return true;
            for (unsigned i = 0; opts->definitions[i].key != nullptr; ++i) {
                const auto& def = opts->definitions[i];
                std::string key(def.key);
                auto& var = variables[key];
                var.key = key;
                var.description = def.desc ? def.desc : key;
                if (var.value.empty() && def.default_value && def.default_value[0] != '\0') {
                    var.value = def.default_value;
                } else if (var.value.empty() && def.values[0].value != nullptr) {
                    var.value = def.values[0].value;
                }
                LOGD("CoreOption v2 %s = %s", key.c_str(), var.value.c_str());
            }
            return true;
        }
        ...
        case RETRO_ENVIRONMENT_SET_CORE_OPTIONS_V2_INTL: {
            ...
            if (data == nullptr) return true;
            const auto* intl = static_cast<const struct retro_core_options_v2_intl*>(data);
            const struct retro_core_options_v2* opts = intl->us;
            if (opts == nullptr || opts->definitions == nullptr) return true;
            for (unsigned i = 0; opts->definitions[i].key != nullptr; ++i) {
                const auto& def = opts->definitions[i];
                std::string key(def.key);
                auto& var = variables[key];
                ...
                LOGD("CoreOption v2_intl %s = %s", key.c_str(), var.value.c_str());
            }
            return true;
        }

        case RETRO_ENVIRONMENT_SET_CORE_OPTIONS_UPDATE_DISPLAY_CALLBACK:
            // We don't implement the dynamic hide/show callback, but returning true
            // prevents cores from falling back to legacy option paths.
            LOGD("Called RETRO_ENVIRONMENT_SET_CORE_OPTIONS_UPDATE_DISPLAY_CALLBACK");
            return true;

        case RETRO_ENVIRONMENT_SET_AUDIO_BUFFER_STATUS_CALLBACK:
            // Some cores (e.g. PCSX-ReARMed) use this to adapt audio output based on
            // buffer fullness. We don't implement the callback but acknowledging it
            // prevents the core from disabling audio on unsupported frontend detection.
            LOGD("Called RETRO_ENVIRONMENT_SET_AUDIO_BUFFER_STATUS_CALLBACK");
            return true;

        case RETRO_ENVIRONMENT_SET_MINIMUM_AUDIO_LATENCY:
            // Acknowledge but do not change latency — Oboe manages this internally.
